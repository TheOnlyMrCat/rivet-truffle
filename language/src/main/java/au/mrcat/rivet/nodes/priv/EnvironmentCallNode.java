...
public class EnvironmentCallNode extends RivetNode {
    ...
    public void executeVoid(VirtualFrame frame) {
        ...
        switch ((int) frame.getLongStatic(RegisterState.A7)) {
            case 64 -> {
                int fd = (int) frame.getLongStatic(RegisterState.A0);
                ...
                if (out == null) {
                    // Return -EBADF
                    frame.setLongStatic(RegisterState.A0, -9);
                    break;
                }

                long ptr = frame.getLongStatic(RegisterState.A1);
                long size = frame.getLongStatic(RegisterState.A2);
                ...
                try {
                    ...
                    // Return EIO
                    frame.setLongStatic(RegisterState.A0, -5);
                    break;
                }

                // Return number of bytes written
                frame.setLongStatic(RegisterState.A0, buf.length);
            }
            case 93 -> throw new RiscvExitException(frame.getLongStatic(RegisterState.A0));
