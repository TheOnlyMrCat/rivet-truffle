...
public class AmoDoubleNode extends RivetOpNode {
    ...
    public long executeLong(VirtualFrame frame, long basePc, PrivilegedContext priv) {
        ...
        try {
            long originalValue = ctx.physicalMemory.readLong(priv.translateReadAddress(address, ctx));

            // Use the zero register as a temporary
            frame.setLongStatic(0, originalValue);
            long opResult = op.executeLong(frame, basePc, priv);

            ctx.physicalMemory.writeLong(priv.translateWriteAddress(address, ctx), opResult);
